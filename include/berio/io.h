...
class octet_output {
   ...
   *
   * It waits until it delivers all the octets requested by the parameter \p len. It gives up when
   * it determines that it will not be able to write them all. In that case, it is guaranteed that
   * no more octets can be written to the stream.
   *
   * Implementations are free to define whether exceptions are thrown.
   *
   * \param optr  Position where the sequence of octets to be written begins.
   * \param len   Number of octets requested to be written.
   *
   * \return The number of octets it has been able to write. It is expected to be less than or equal
   *         to \p len.
   */
