 ...
 * \file
 *
 * \brief Tag header structure and its basic operations
 *
 * Enables encoding, decoding, and obtaining the length needed to encode the header of a tag.
 */
 ...
 *
 * If the size is not sufficient for an unexpected case, the tag_decode(tag_header&, octet_input&,
 * bool) operation returns ber::tdr_number_overflow.
 *
 * \see tag_header::number
 *
 * \ingroup tag-header
 */
using tag_number = unsigned short int;
...
enum tag_shape {
   ...
   *
   * None of the tags present in the content can satisfy univ::is_eoc(tag_header const&, bool).
   */
   ...
   *
   * The last tag present in the content must satisfy univ::is_eoc(tag_header const&, bool).
   */
 ...
 * length or not.
 *
 * \see tag_decode(tag_header&, octet_input&, bool)
 * \see tag_sizeof(tag_header const&)
 * \see tag_encode(tag_header const&, octet_output&)
 *
 * \ingroup tag-header
 */
struct tag_header {
 ...
 *
 * Exceptions thrown during read operations on \p in are not inhibited. If this happens, the
 * original value of \p th is retained.
 *
...
static_assert(
  std::numeric_limits<std::size_t>::max() >= 10 + (std::numeric_limits<tag_number>::digits + 6) / 7,
  "The maximum size of a tag header exceeds the allowed size");

/*!
 * \brief Length required to encode a tag header
 *
 * This operation is consistent with tag_encode(tag_header const&, octet_output&).
 *
 * \pre <b>Static assertion</b>:
 ...
 * \ingroup tag-header
 */
std::size_t tag_sizeof(tag_header const& th) noexcept;

static_assert(sizeof(std::size_t) < 128, "X.609 does not support lengths greater than 2^127");

/*!
 * \brief Encode a tag header
 *
 * The return value \c true indicates that the encoding has been completed. The operation is
 * interrupted with a return value of \c false the moment it becomes impossible to write to the
 * output stream specified in the \c out parameter. In any case, the writing of the octets is not
 * reversed.
 *
 * This operation is consistent with tag_sizeof(tag_header const&).
 *
 * \pre <b>Static assertion</b>:
 * The maximum value of std::size_t is less than 2<sup>127</sup>. See ber::tdr_length_unsupported.
 *
 * The following program
 * \include tag-encode.cxx
 *
 * produces the output
 * \include tag-encode.out
 *
 * Exceptions thrown during write operations on \p out are not inhibited.
 *
 * \param th   Tag header to be encoded.
 * \param out  Stream in which the encoded result is written.
 *
 * \return \c true if the encoding could be completed; \c false otherwise.
 *
 * \ingroup tag-header
 */
bool tag_encode(tag_header const& th, octet_output& out);
