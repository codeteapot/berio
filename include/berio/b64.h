 ...
 * \file
 *
 * \brief Input and output streams for Base-64 and their respective canonical traits
 *
 * It contains the ber::octet_input and ber::octet_output adapters for Base-64 character sources and
 * sinks respectively.
 */
...
/*!
 * \brief Reveal and discard the next element from a character source
 *
 * The source is delegated the responsibility of holding the next character even after it has been
 * read. This character may contain bits that will be relevant for the subsequent octet.
 *
 * \note
 * It is designed taking into account std::basic_istream. See \ref in-out-base64-stdlib.
 *
 * This is a canonical template. If no specialization for \p SourceT is found, a compilation error
 * will occur.
 *
 * \tparam SourceT  The type of character source.
 *
...
struct b64_source_traits {
  ...
  /*!
   * \brief Reveal the next character of the source without discarding it
   *
   * Stores the next character in \p c and returns \c true if there are characters available in
   * \p src. Returns \c false otherwise.
   *
   * \pre **Static assertion**:
   * This method needs to be specialized.
   *
   * The b64_source_traits::ignore(SourceT&) method is called to move to the next character.
   *
   * \param src  The source from which the next character is obtained.
   * \param c    Reference in which the obtained character is stored.
   *
   * \return \c true if, and only if, there was an available character; \c false otherwise.
   */
  ...
  /*!
   * \brief Discard the next character from the character source
   *
   * \pre **Static assertion**:
   * This method needs to be specialized.
   *
   * It is used when the character obtained by b64_source_traits::peek(SourceT&, char&) is no longer
   * needed.
   *
   * \param src  The source whose next character should be discarded.
   */
...
/*!
 * \brief Insert a new element into a character sink
 *
 * \note
 * It is designed taking into account std::basic_ostream. See \ref in-out-base64-stdlib.
 *
 * This is a canonical template. If no specialization for \p SinkT is found, a compilation error
 * will occur.
 *
 * \tparam SinkT  The type of character sink.
 *
 * \ingroup in-out-base64
 */
...
struct b64_sink_traits {
  ...
  /*!
   * \brief Insert a character into the sink
   *
   * Use \c c as the character to insert into \p sink and return \c true if it was possible. Return
   * \c false otherwise.
   *
   * \pre **Static assertion**:
   * This method needs to be specialized.
   *
   * \param sink  The sink into which the character is inserted.
   * \param c     The value of the character to be inserted.
   *
   * \return \c true if, and only if, it has been possible to insert the character; \c false
   *         otherwise.
   */
...
/*!
 * \brief Input stream for a Base-64 character source
 *
 * It wraps a character source of type \p SourceT that is treated as defined in \p Traits.
 *
 * \note
 * It also implements delimited_input::skippable, so it can be wrapped in turn by a delimited_input.
 *
 * The b64_source_traits template is simply the canonical form of \p Traits. It displays the
 * behavior of \p SourceT, but requires specialization.
 *
 * Characters for which std::isspace(int) evaluates to \c true are automatically discarded.
 *
 * \tparam SourceT  Type of the wrapped character source.
 * \tparam Traits   Operations applied to the character source.
 *
 * \ingroup in-out-base64
 */
template<typename SourceT, typename Traits = b64_source_traits<SourceT>>
class b64_input : public delimited_input::skippable {

  public:
  
  /*!
   * \brief Wraps an existing character source
   *
   * The initial state for the Base-64 to octet conversion is established.
   *
   * \pre
   * The next character from the source is the first of the quartet. Risk of misreading.
   *
   * \param src  The wrapped character source.
   */
   ...
   *
   * Preservation of the conversion state is not implemented.
   */
  b64_input(b64_input const&) = delete;
  ...
  std::size_t skip(std::size_t len) override final {
    unsigned char null;
    std::size_t actual_len = 0;
...
class b64_output : public octet_output {
  ...
  b64_output(b64_output const&) = delete;

  std::size_t putn(unsigned char const* optr, std::size_t len) override final {
    std::size_t actual_len = 0;
