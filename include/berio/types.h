 ...
 */
template<typename BoolT, typename Traits = boolean_traits<BoolT>>
std::size_t tag_sizeof_boolean(BoolT const& b) noexcept {
  return 1;
}
 ...
 */
template<typename BoolT, typename Traits = boolean_traits<BoolT>>
void tag_encode_boolean(BoolT const& b, octet_output& out) {
...
tag_decode_integer_result tag_decode_integer(IntT& i, octet_input& in) {
  i = Traits::zero();
  unsigned char o;
  if (not in.get(o))
    return tdir_empty;
...
tag_decode_bit_string_result tag_decode_bit_string(BitStringT& bstr, octet_input& in) {
  unsigned char o;
  if (not in.get(o))
    return tdbsr_empty;
  // TODO Apply padding
...
struct octet_string_traits {
  ...
  static size_type size(OctetStrT const& ostr) noexcept { return ostr.size(); }
...
tag_decode_octet_string_result tag_decode_octet_string(OctetStrT& ostr, octet_input& in) {
  unsigned char o;
  if (not in.get(o))
    return tdosr_empty;
  do
 ...
 */
template<typename OctetStrT, typename Traits = octet_string_traits<OctetStrT>>
std::size_t tag_sizeof_octet_string(OctetStrT const& ostr) noexcept {
 ...
 */
template<typename OctetStrT, typename Traits = octet_string_traits<OctetStrT>>
void tag_encode_octet_string(OctetStrT const& ostr, octet_output& out) {
...
tag_decode_utf8_string_result tag_decode_utf8_string(UTF8StrT& u8str, octet_input& in) {
  // TODO Implement
...
tag_decode_print_string_result tag_decode_print_string(PrintStrT& pstr, octet_input& in) {
  unsigned char o;
  if (not in.get(o))
    return tdpsr_empty;
  do
...
tag_decode_oid_result tag_decode_oid(OIDT& oid, octet_input& in) {
