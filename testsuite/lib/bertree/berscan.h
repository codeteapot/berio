...
template<typename CharT, typename Traits>
void ber_scan(ber::delimited_input& in, std::basic_ostream<CharT, Traits>& os) {
  ber_scan(in, os, 0);
}
...
ber_scan_result ber_scan(
  ...
  if (tdr == ber::tdr_complete) {
    ...
    if (not (os << level << ' ' << ber_label(th.tclass, th.number, definite)))
      return bsr_failed;
    ber::delimited_input::sentry delimit(in, th.shape == ber::ts_constructed_indefinite
      ? ber::delimited_input::max_length
      : th.length);
...
void ber_scan_integer(ber::delimited_input& in, std::basic_ostream<CharT, Traits>& os) {
  ber_integer i;
  ber::tag_decode_integer_result tdir = ber::tag_decode_integer(i, in);
...
void ber_scan_bit_string(ber::delimited_input& in, std::basic_ostream<CharT, Traits>& os) {
  ber_bit_string bstr;
  ber::tag_decode_bit_string(bstr, in);
  os << bstr;
}
...
void ber_scan_oid(ber::delimited_input& in, std::basic_ostream<CharT, Traits>& os) {
  ber_oid oid;
  ber::tag_decode_oid_result tdoidr = ber::tag_decode_oid(oid, in);
...
void ber_scan_print_string(ber::delimited_input& in, std::basic_ostream<CharT, Traits>& os) {
  ...
  ber::tag_decode_print_string(pstr, in);
  os << pstr;
}
...
void ber_scan_octet_string(ber::delimited_input& in, std::basic_ostream<CharT, Traits>& os) {
  ...
  ber::tag_decode_octet_string(ostr, in);
  os << ostr;
}
