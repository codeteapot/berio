...
class stub {
  ...
  constexpr explicit stub(Whens&&... whens):
  __whens(std::forward<Whens>(whens)...) {}
...
class stub<> {
  ...
  constexpr explicit stub() {}
};

template<typename... Whens>
stub(Whens&&...) -> stub<std::decay_t<Whens>...>;
...
class when {
  ...
  constexpr explicit when(CalledWith&& called_with, Answer&& answer):
  __called_with(std::forward<CalledWith>(called_with)),
  __answer(std::forward<Answer>(answer)) {}
...
template<typename CalledWith, typename Answer>
when(CalledWith&&, Answer&&) -> when<std::decay_t<CalledWith>, std::decay_t<Answer>>;
...
class is_called_with {
  ...
  constexpr explicit is_called_with(Matchers&&... matchers):
  __matchers_pack(std::forward<Matchers>(matchers)...) {}
...
template<typename... Matchers>
is_called_with(Matchers&&...) -> is_called_with<std::decay_t<Matchers>...>;
...
template<typename>
struct that_arg_type;

template<typename C, typename R, typename Arg>
struct that_arg_type<R(C::*)(Arg) const> {
  using type = Arg;
};
...
class that {
  ...
  using type = typename that_arg_type<decltype(&P::operator())>::type;
  
  constexpr that(P pred):
  __pred(pred) {}
