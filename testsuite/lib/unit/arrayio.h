...
template<std::size_t Size>
class array_octet_input : public ber::delimited_input::skippable {
  ...
  bool get(unsigned char& o) override {
    if (__pos < Size) {
      o = __arr[__pos++];
      return true;
    }
  ...
  std::size_t getn(unsigned char* optr, std::size_t len) override {
    std::size_t count = 0;
...
class array_octet_output : public ber::octet_output {
  ...
  std::size_t putn(unsigned char const* optr, std::size_t len) override {
    std::size_t count = 0;
    while (count < len and __pos < Size)
