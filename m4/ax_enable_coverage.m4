...
coverage.info: coverage-init check
	...
	# GCC emits D0/D2 destructor aliases. Some LCOV versions count uncovered D0 aliases as
	# independent functions, reducing function coverage spuriously. Remove those aliases from
	# the tracefile.
	sed -i '/^FNA:[[0-9]]*,0,.*D0Ev\$\$/d' coverage.info
